...
async def chat(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    ...
    receive = None
    writer.write(
        f"Hello, it is cow chat. Command list: {' '.join(commands)}\n".encode()
    )
    await writer.drain()
    while not reader.at_eof():
        ...
        for q in done:
            ...
            if command not in commands:
                writer.write("Unknown command, please try again\n".encode())
                await writer.drain()
                continue
            ...
            match command:
                ...
                case "login":
                    if me in addr_to_client:
                        writer.write("You have already logged in\n".encode())
                        await writer.drain()
                        continue
                    if len(args) == 0:
                        writer.write("You have to specify cow name\n".encode())
                        await writer.drain()
                        continue
                    cow_name = args[0]
                    if cow_name in clients:
                        writer.write(
                            "User with this cow name already exists. Use other cow name\n".encode()
                        )
                        await writer.drain()
                        continue
                    if cow_name not in cowsay.list_cows():
                        writer.write(
                            "Unknown cow name, try one of available\n".encode()
                        )
                        await writer.drain()
                        continue
                    ...
                    receive = asyncio.create_task(clients[addr_to_client[me]].get())
                    writer.write("Successful log in, welcome to chat\n".encode())
                    await writer.drain()

                case "say":
                    if me not in addr_to_client:
                        writer.write("Login is required for this command\n".encode())
                        await writer.drain()
                        continue
                    if len(args) != 2:
                        writer.write(
                            "Invalid value of command arguments, must be 2 arguments\n".encode()
                        )
                        await writer.drain()
                        continue
                    send_to, text = args
                    if send_to not in clients:
                        writer.write(
                            f"User with username {send_to} does not exist\n".encode()
                        )
                        await writer.drain()
                        continue
                ...
                case "yield":
                    ...
                    if len(args) < 1:
                        writer.write(
                            "Invalid value of command arguments, must be 1 argument\n".encode()
                        )
                        await writer.drain()
                        continue
                    text = args[0]
                    ...
                    for queue in clients.values():
                        tasks.append(asyncio.create_task(queue.put(text)))
